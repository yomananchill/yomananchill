...
`assets/night-shift.gif` is a 1200 × 400, 12 fps, six-second loop composited from the original illustration using FFmpeg. Rain is confined to the window, and CRT illumination changes slowly at low amplitude. The camera, character, and typography remain static. Run `node assets/animate-banner.mjs` to rebuild. The README serves the still PNG when reduced motion is requested.
